...
def register_and_login(base_url: str, user_data: dict) -> dict:
    ...
    ), f"User registration failed: {reg_response.text}"
...
def test_create_calculation_addition(base_url: str):
    ...
    assert (
        response.status_code == 201
    ), f"Addition calculation creation failed: {response.text}"
    data = response.json()
    assert (
        "result" in data and data["result"] == 15.5
...
def test_create_calculation_subtraction(base_url: str):
    ...
    assert (
        response.status_code == 201
    ), f"Subtraction calculation creation failed: {response.text}"
    data = response.json()
    # Expected result: 10 - 3 - 2 = 5
    assert (
        "result" in data and data["result"] == 5
...
def test_create_calculation_multiplication(base_url: str):
    ...
    assert (
        response.status_code == 201
    ), f"Multiplication calculation creation failed: {response.text}"
    data = response.json()
    # Expected result: 2 * 3 * 4 = 24
    assert (
        "result" in data and data["result"] == 24
...
def test_create_calculation_division(base_url: str):
    ...
    assert (
        response.status_code == 201
    ), f"Division calculation creation failed: {response.text}"
    data = response.json()
    # Expected result: 100 / 2 / 5 = 10
    assert (
        "result" in data and data["result"] == 10
...
def test_list_get_update_delete_calculation(base_url: str):
    ...
    assert any(
        c["id"] == calc_id for c in calc_list
    ...
    ), f"Get calculation failed: {get_response.text}"
    ...
    expected_result = 30
    assert (
        updated_calc["result"] == expected_result
...
def test_create_calculation_exponentiation(base_url: str):
    ...
    assert (
        response.status_code == 201
    ), f"Exponentiation creation failed: {response.text}"
    data = response.json()
    assert (
        "result" in data and data["result"] == 8
    ), f"Expected result 8, got {data.get('result')}"
...
def test_model_exponentiation_invalid_inputs():
    dummy_user_id = uuid4()
    with pytest.raises(ValueError):
        calc = Calculation.create(
            "exponentiation", dummy_user_id, [2]
        )  # only one input
        calc.get_result()
