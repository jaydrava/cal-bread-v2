...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            -3,
            -2,
        ),  # Test subtracting a negative integer from another negative integer
...
def test_exponentiation(a: float, b: float, expected: float) -> None:
    result = exponentiation(a, b)
    assert (
        result == expected
    ), f"Expected exponentiation({a}, {b}) to be {expected}, but got {result}"
