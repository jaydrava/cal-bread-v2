...
def test_invalid_inputs_for_subtraction():
    ...
    with pytest.raises(
        ValueError, match="Inputs must be a list with at least two numbers."
    ):
...
def test_exponentiation_invalid_inputs_type():
    """
    Test that Exponentiation.get_result raises ValueError if inputs are not a list.
    """
...
def test_exponentiation_invalid_inputs_length():
    """
    Test that Exponentiation.get_result raises ValueError if inputs length is not 2.
    """
    exponentiation = Exponentiation(user_id=dummy_user_id(), inputs=[2])
    with pytest.raises(
        ValueError, match="Exponentiation requires exactly two numbers."
    ):
        exponentiation.get_result()
